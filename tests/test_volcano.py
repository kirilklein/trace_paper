...
class TestPrepareVolcanoData(unittest.TestCase):
    ...
    def test_adjust_by_method(self):
        ...
        result = prepare_volcano_data(self.df, adjust="bh", adjust_per="by_method")
        ...
        for method in ["IPW", "TMLE"]:
            ...
            self.assertTrue(
                np.all(method_data["q_value"].values >= method_data["p_value"].values)
            )

    def test_adjust_global(self):
        """Test global adjustment."""
        result = prepare_volcano_data(self.df, adjust="bh", adjust_per="global")
    ...
    def test_no_adjustment(self):
        ...
        result = prepare_volcano_data(self.df, adjust="none")

        # q_value should equal p_value
        np.testing.assert_array_almost_equal(
            result["q_value"].values, result["p_value"].values
        )
    ...
    def test_custom_column_names(self):
        ...
        result = prepare_volcano_data(
            df, rd_col="effect", p_col="pval", method_col="meth", outcome_col="out"
        )
...
class TestVolcanoPlot(unittest.TestCase):
    ...
    def test_label_map_applied(self):
        ...
        # Should not raise error
        fig, axes = volcano_plot_per_method(self.volcano_data, label_map=label_map)

        plt.close(fig)

    def test_custom_figsize(self):
        """Test custom figure size."""
        fig, axes = volcano_plot_per_method(self.volcano_data, figsize_per_panel=(8, 6))
    ...
    def test_custom_colors(self):
        ...
        fig, axes = volcano_plot_per_method(
            self.volcano_data, sig_color="red", ns_color="gray"
        )
    ...
    def test_max_labels_limits_annotations(self):
        ...
        fig1, _ = volcano_plot_per_method(self.volcano_data, max_labels_per_panel=3)
        fig2, _ = volcano_plot_per_method(self.volcano_data, max_labels_per_panel=20)
    ...
    def test_no_methods_raises_error(self):
        ...
        empty_df = pd.DataFrame(
            columns=["method", "outcome", "RD", "p_value", "q_value", "neglog10p"]
        )
